...
Gameplay visuals derive approach, fade, judgement, and follow-circle animation values from the snapshot's GameClock time and object timing. `GameplayState` also carries short-lived judgement visuals emitted beside their existing judgement events; these do not affect ScoreTracker. `GameplayRenderer` draws slider bodies from the existing sampled SliderPath and caches segment meshes, cumulative distances, and bounds while a Slider is visible. The osu! renderer resolves the minimum centreline distance in a shared GPU buffer and composites each legacy body once at its render-plan depth; its profile and skin.ini Colours are documented in [Slider Body parity](slider-body-parity.md). Colors and default visual dimensions live in `GameplayVisualConfig`, separate from ruleset logic. `GameplayHudRenderer` draws combo numbers, 300/100/50/Miss feedback, and the score, accuracy, and combo HUD.
